...
function SectionHeading({ title, to, slug }: { title: string; to?: string; slug?: string }) {
  return (
    <div className="mb-6 flex items-center justify-between border-b-2 border-foreground/90 pb-2.5">
      <div className="flex items-center gap-2.5">
        <span className="h-4 w-1.5 bg-crimson" />
        <h2 className="font-serif text-2xl md:text-3xl font-extrabold tracking-tight text-foreground">{title}</h2>
      </div>
      {(to || slug) && (
        <Link
          to={slug ? "/category/$slug" : (to as never)}
          params={slug ? { slug } : undefined}
          className="meta flex items-center gap-1 text-crimson hover:underline"
        >
...
function Home() {
  ...
  const rest = articles.filter((a) => a.id !== lead?.id && !secondaries.includes(a));
  ...
  const previewSections = categories.filter((c) =>
    ["business", "sports", "entertainment", "tech"].includes(c.slug),
  );
  ...
  const modules: Record<string, ReactElement | null> = {
    hero:
      settings?.hero_takeover_enabled && settings.hero_takeover_url ? (
        <section className="mx-auto max-w-[1400px] px-4 pt-6 md:pt-8">
          <div className="relative overflow-hidden rounded-md border border-border shadow-lift">
            <img
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/95 via-black/60 to-transparent p-6 md:p-10 text-white">
              <span className="meta text-gold mb-2 block font-extrabold">Special Coverage</span>
        ...
        <section className="mx-auto max-w-[1400px] px-4 pt-6 md:pt-8">
          <div className="grid gap-8 lg:grid-cols-12 items-start border-b border-border pb-10">
            {/* Main Lead Story (7 Cols) */}
            <div className="lg:col-span-7 border-b lg:border-b-0 lg:border-r border-border/80 lg:pr-8 pb-8 lg:pb-0">
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="flex items-center gap-2 border-b border-border pb-2">
                ...
                <h3 className="meta font-extrabold text-foreground tracking-widest">Key Newsroom Stories</h3>
              </div>
              {secondaries[0] && <SecondaryLeadCard article={secondaries[0]} />}
              <div className="space-y-1">
                {secondaries.slice(1).map((a) => (
    ...
    categories: (
      <section className="mx-auto max-w-[1400px] px-4 pt-8">
        <div className="flex items-center gap-2 border-b border-border pb-3 overflow-x-auto [scrollbar-width:none]">
          <span className="meta text-muted-foreground shrink-0 font-bold mr-2 flex items-center gap-1">
          ...
          <aside className="lg:col-span-4 lg:border-l lg:border-border lg:pl-8 space-y-8">
            ...
            <div className="rounded-md border border-border bg-card p-5 shadow-rest">
              <div className="flex items-center gap-2 border-b border-border pb-3 mb-3">
                ...
                <h3 className="font-serif text-xl font-bold">Most Read in Delhi</h3>
              </div>
    ...
    delhi: (
      <section className="mt-14 border-y border-border bg-secondary/30 py-12 md:py-16">
        <div className="mx-auto max-w-[1400px] px-4">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
            <div>
              <span className="meta text-crimson font-extrabold tracking-widest">Hyperlocal Coverage</span>
              <h2 className="mt-1 font-serif text-3xl md:text-4xl font-extrabold">Delhi / NCR Spotlight</h2>
            </div>

            {/* Weather / AQI Banner Widget */}
            <div className="flex items-center gap-4 rounded-md border border-border bg-card px-5 py-3 shadow-rest text-xs font-sans">
              <div className="flex items-center gap-2.5">
                <div className="grid h-8 w-8 place-items-center rounded-full bg-crimson/10 text-crimson">
    ...
    video: videos.length ? (
      <section className="mx-auto max-w-[1400px] px-4 pt-14">
    ...
    opinion: opinion.length ? (
      <section className="mx-auto max-w-[1400px] px-4 pt-14">
        <SectionHeading title="Opinion & Columns" slug="opinion" />
    ...
    sections: (
      <div className="mx-auto max-w-[1400px] px-4">
        {previewSections.map((c) => {
          ...
          return (
            <section key={c.id} className="pt-14">
    ...
    newsletter: (
      <section className="mx-auto mt-16 max-w-[1400px] px-4 mb-14">
        <div className="rounded-md border border-border bg-card p-8 md:p-12 text-center shadow-lift max-w-3xl mx-auto border-t-4 border-t-crimson">
          <span className="meta text-crimson font-extrabold tracking-widest">The Morning Brief</span>
          <h2 className="mt-2 font-serif text-2xl md:text-4xl font-extrabold">Delhi, decoded before 8 am</h2>
          <p className="mx-auto mt-3 max-w-lg text-xs md:text-sm text-muted-foreground leading-relaxed">
            Essential morning dispatches delivered straight from the Delhi News Live newsroom. Curated coverage of policy, city affairs, and regional pulse.
          </p>
          <form
            className="mx-auto mt-6 flex max-w-md flex-col sm:flex-row gap-2.5"
          ...
          >
            <input
              ...
              className="flex-1 rounded-md border border-border bg-background px-4 py-2.5 text-sm outline-none focus:border-crimson focus:ring-1 focus:ring-crimson transition-all"
            />
            <button className="rounded-md bg-crimson px-6 py-2.5 text-sm font-bold text-crimson-foreground transition-all hover:opacity-95 shadow-sm">
              Subscribe Free
            </button>
          </form>
        </div>
...
function FilterPill({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "shrink-0 rounded-md px-3 py-1.5 text-xs font-semibold transition-all duration-150",
        active
          ? "bg-crimson text-crimson-foreground shadow-sm"
          : "text-foreground/80 hover:bg-accent hover:text-foreground border border-border/60",
      )}
    >
      {label}
    </button>
